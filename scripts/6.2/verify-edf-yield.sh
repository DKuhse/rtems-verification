#!/bin/bash
#
# Verify _Scheduler_EDF_Yield on the active RTEMS 6.2 port.
#
# Two WP passes:
#   1. function goals for _Scheduler_EDF_Yield + trivially-verifiable helpers
...
run_fc() {
    ...
    ${FRAMA_C_CMD} \
        ...
        -inline-calls "_Scheduler_uniprocessor_Yield,_Scheduler_uniprocessor_Update_heir_if_necessary" \
        "${SRC}" \
        ${defaults} \
        "$@"
}
...
if [ "${GUI}" = "1" ]; then
    ...
    echo "=== EDF Yield (RTEMS 6.2 active port): function ==="
    run_fc "${WP_FCT_DEFAULTS}" "$@"
    echo "=== EDF Yield (RTEMS 6.2 active port): model lemma ==="
