#!/bin/bash
#
# Composite runner for the active RTEMS 6.2 verification scripts.
#
# Runs every verify-*.sh in this directory in turn, parses each WP
...
# Documented active scripts, in the order they appear in README.md.
SCRIPTS=(
    verify-edf-block.sh
...
if [ "${INCLUDE_TMP}" = "1" ]; then
    SCRIPTS+=(tmp-verify-thread-get-priority.sh)
fi
