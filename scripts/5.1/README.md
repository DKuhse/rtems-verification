# RTEMS 5.1 Verification Scripts
...
All run against the FC 32 stack via `docker compose run --rm verify-5.1-fc32`.
The scripts also detect FC 25 and switch the C standard flag from `-std c11`
to `-c11` so quick compatibility checks can run on `verify-5.1`.

- `verify-edf-initialize.sh` — runs Frama-C/WP on the
  `_Scheduler_EDF_Initialize()` slice and checks that initialization produces
  an empty, well-formed EDF ready context.
- `verify-edf-block.sh` — verifies the `_Scheduler_EDF_Block()` slice.
- `verify-edf-update-priority.sh` — verifies the
...
The 5.1 scripts share Stage 1 (RTEMS cross-toolchain image, AMD64 BSP)
with the existing `verify` service. The Frama-C stack is the same as the
reference 6.2 port:

| stack | service | scripts | purpose |
|---|---|---|---|
| Frama-C 25.0 + Alt-Ergo 2.4.2 | `verify-5.1` | this directory | byte-reproducibility on legacy stack (limited) |
| Frama-C 32.0 + Alt-Ergo 2.6.x | `verify-5.1-fc32` | this directory | 5.1 work, EDF property proofs |
