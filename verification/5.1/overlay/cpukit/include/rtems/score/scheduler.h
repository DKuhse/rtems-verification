...
typedef struct Scheduler_Context {
  ...
  /*
   * In 5.1 non-SMP, ISR_LOCK_MEMBER( Lock ) expands to nothing, leaving
   * this struct empty.  Empty structs trip WP's typed_cast model and hang
   * goal generation when Scheduler_Context appears as the first field of
   * Scheduler_EDF_Context.  Mirror the 6.2 layout under Frama-C only.
   */
  char empty;
#endif
} Scheduler_Context;
