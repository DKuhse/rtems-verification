...
(* Why3 goal *)
Theorem wp_goal :
  forall (t:addr -> addr) (a:addr) (a1:addr),
  let a2 := t (shift (shift a 0%Z) 1%Z) in
  ~ (null = a) -> ~ (a2 = null) -> P_in_subtree t a1 a ->
  P_in_subtree t a1 a2.
Proof.
intros t a a1 a2 h1 h2 h3.
induction h3.
- (* Q_in_here: root = a, apply Q_in_right then Q_in_here. *)
  apply Q_in_right; [ exact h1 | ].
  apply Q_in_here.
  intro Heq; apply h2; symmetry; exact Heq.
- (* Q_in_left: IH applies. *)
  apply Q_in_left; [ assumption | ].
  apply IHh3; assumption.
- (* Q_in_right: symmetric. *)
  apply Q_in_right; [ assumption | ].
  apply IHh3; assumption.
Qed.
