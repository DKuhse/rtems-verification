...
Theorem wp_goal :
  forall (t:addr -> addr) (i:Numbers.BinNums.Z) (i1:Numbers.BinNums.Z)
    (a:addr) (a1:addr),
  let x := L_key a in
  P_in_subtree t a1 a -> P_bst_order t a1 i1 i -> (i1 < x)%Z /\ (x < i)%Z.
Proof.
intros t i i1 a a1 x h1 h2.
revert i i1 h2.
induction h1 as [Mptr root Hnn | Mptr root n Hnn hlr IH | Mptr root n Hnn hrr IH];
  intros i i1 h2.
- (* Q_in_here: root = a, apply Q_bst_order_key_in_bounds directly. *)
  apply (Q_bst_order_key_in_bounds Mptr root i1 i Hnn h2).
- (* Q_in_left: n in left subtree of root. *)
  pose proof (Q_bst_order_left Mptr root i1 i Hnn h2) as Hleft.
  pose proof (IH (L_key root) i1 Hleft) as [Hlo Hhi].
  pose proof (Q_bst_order_key_in_bounds Mptr root i1 i Hnn h2) as [_ Hkr_lt_i].
  split; [ exact Hlo | ].
  apply Z.lt_trans with (m := L_key root); assumption.
- (* Q_in_right: symmetric using Q_bst_order_right. *)
  pose proof (Q_bst_order_right Mptr root i1 i Hnn h2) as Hright.
  pose proof (IH i (L_key root) Hright) as [Hlo Hhi].
  pose proof (Q_bst_order_key_in_bounds Mptr root i1 i Hnn h2) as [Hi1_lt_kr _].
  split; [ | exact Hhi ].
  apply Z.lt_trans with (m := L_key root); assumption.
Qed.
