...
Axiom Q_bst_order_subtree_member_ordered :
  forall (Mptr:addr -> addr) (root:addr) (n:addr) (lo:Numbers.BinNums.Z)
    (hi:Numbers.BinNums.Z),
  P_in_subtree Mptr root n -> P_bst_order Mptr root lo hi ->
  P_bst_order Mptr n lo hi.
...
Axiom Q_parent_linked_member_parent_valid :
  forall (Malloc:Numbers.BinNums.Z -> Numbers.BinNums.Z) (Mptr:addr -> addr)
    (root:addr) (n:addr),
  ~ (root = n) -> P_parent_linked Mptr root null ->
  P_in_subtree Mptr root n ->
  valid_rd Malloc (Mptr (shift (shift n 0%Z) 2%Z)) 4%Z.

Axiom Q_parent_linked_member_parent_in_tree :
  forall (Mptr:addr -> addr) (root:addr) (n:addr), ~ (root = n) ->
  P_parent_linked Mptr root null -> P_in_subtree Mptr root n ->
  P_in_subtree Mptr root (Mptr (shift (shift n 0%Z) 2%Z)).
...
Theorem wp_goal :
  ...
  P_in_subtree t1 a1 a -> P_wf_node t t1 a1 i -> valid_rd t a 4%Z.
Proof.
intros t t1 i a a1 h1 h2.
revert i h2.
induction h1 as [Mptr root Hnn | Mptr root n Hnn hlr IH | Mptr root n Hnn hrr IH];
  intros d Hwf.
- (* Q_in_here: a1 = a = root. Apply Q_wf_node_valid directly. *)
  apply (Q_wf_node_valid t Mptr root d Hnn Hwf).
- (* Q_in_left: n in left subtree. Apply IH. *)
  pose proof (Q_wf_node_depth_positive t Mptr root d Hnn Hwf) as Hd_pos.
  pose proof (Q_wf_node_left t Mptr root d Hnn Hd_pos Hwf) as Hleft.
  apply (IH ((-1)%Z + d)%Z Hleft).
- (* Q_in_right: symmetric. *)
  pose proof (Q_wf_node_depth_positive t Mptr root d Hnn Hwf) as Hd_pos.
  pose proof (Q_wf_node_right t Mptr root d Hnn Hd_pos Hwf) as Hright.
  apply (IH ((-1)%Z + d)%Z Hright).
Qed.
