...
This note records the current boundary decision for the EDF release-job work.
...
release-job contract produces the queue-context state consumed by
`_Thread_Priority_update()`, and that update restores/preserves the EDF ready
context while leaving the modeled ready set unchanged.
